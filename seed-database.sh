...
if [ -f "$DB_FILE" ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo "Aborted. Use a different database file or delete the existing one."
        exit 1
    fi
fi
...
INSERT INTO users (id, username, email, name, password_hash, salt, is_admin, created_at) VALUES
    ('$USER1_ID', 'alice', 'alice@example.com', 'Alice Johnson', '$PASSWORD_HASH', '$SALT', 0, '$NOW'),
    ('$USER2_ID', 'bob', 'bob@example.com', 'Bob Smith', '$PASSWORD_HASH', '$SALT', 0, '$NOW'),
    ('$USER3_ID', 'charlie', 'charlie@example.com', 'Charlie Brown', '$PASSWORD_HASH', '$SALT', 0, '$NOW'),
    ('$ADMIN_ID', 'admin', 'admin@example.com', 'Admin User', '$PASSWORD_HASH', '$SALT', 1, '$NOW');
