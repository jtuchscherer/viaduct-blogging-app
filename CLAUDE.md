...
### Viaduct 2.0 features: what was evaluated and not adopted

From an audit of the [2.0.0 release notes](https://github.com/airbnb/viaduct/releases/tag/v2.0.0). Named fragments, property-based tests and the schema snapshot check were adopted, and each explains itself where it lives — `CheckedListFragments.kt`, `SuggestChecklistItemPropertyTest.kt`, `schema-check.sh`. Recorded here are only the findings that produced no code, and so have nowhere else to live.

- **`@parent` fields — rejected, does not apply here.** The directive resolves a field to the *current execution parent object*, not to a column on the parent row, which is the natural misreading of the release note. Even read correctly it cannot serve `CheckedListItem`: build validation fails with `PARENT_FIELD_CHILD_HAS_AMBIGUOUS_PARENT`, because `@parent` requires exactly one producer field and items have four (`CheckedListPost.items` plus three mutations). The `getAuthorIdsByPostIds` round-trip stays. Worth reconsidering only for a type produced by exactly one field.
- **Viaduct's `Arb.viaduct` / `CheckedArb` generators are deliberately unused.** They generate arbitrary schemas, queries and runtime wirings to fuzz the *engine*; property-testing our own resolver argument validation needs none of that, so the property tests use kotest-property directly.
- **Viaduct's "semantic binary schema diff" is not the tool for contract checks.** It is internal artifact-compatibility tooling, not consumer-facing, which is why `schema-check.sh` is built on introspection instead.
